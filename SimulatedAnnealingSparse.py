...
MUX_DELAY_MAX_NS = {
    ...
    32:0.34,33:0.34,36:0.31,37:0.35,40:0.31,35:0.32,43:0.38,45:0.38,46:0.38,48:0.37,49:0.36, 51:0.37, 52:0.37, 54:0.38, 58:0.4,
    ...
    206:0.32,208:0.36,215:0.36,222:0.37,238:0.33,254:0.28,256:0.28,260:0.32,288:0.31,
...
def build_pae_has_ffs(bind_vars_info, connect_vars_info, ff_list, paes):
    """
...
def generate_bitstream(
    ...
    引数:
        bind_vars_info: 配置情報
        connect_vars_info: 配線情報
        pae_has_ffs: PAEのFlipFlop利用情報
        logic_to_mode: PAEのMODE情報
        clamp_info: クランプ情報
        netlist_file: .netファイルパス
        mapped_v_path: mapped.vファイルパス
        connection_xml_path: eFPGAConnection.xmlファイルパス
        output_bit_path: 出力ビットストリームファイルパス
        paes: PAEオブジェクトのリスト
        external_inputs: 外部入力の辞書
        external_outputs: 外部出力の辞書
        ff_list: FFリスト
    ...
    # srcディレクトリのパスを追加
    # 現在のファイル位置から相対的にsrcディレクトリを探す
    # 現在のファイル: .../src/BitstreamGen/SAPR/SimulatedAnnealingSparse.py
    # 目標: .../src ディレクトリ
    current_file = Path(__file__).resolve()
    
    # 現在のファイルから親ディレクトリを遡ってsrcディレクトリを探す
    src_path = None
    search_path = current_file.parent  # BitstreamGen/SAPR/
    
    # 最大10階層まで遡ってsrcディレクトリを探す
    for _ in range(10):
        # srcディレクトリの特徴: BitstreamGenサブディレクトリが存在する
        if (search_path / "BitstreamGen").exists() or (search_path.parent / "BitstreamGen").exists():
            # search_pathがsrcディレクトリ、またはその親
            if (search_path / "BitstreamGen").exists():
                src_path = search_path
            elif (search_path.parent / "BitstreamGen").exists():
                src_path = search_path.parent
            break
        search_path = search_path.parent
        if search_path == search_path.parent:  # ルートディレクトリに到達
            break
    
    # 見つからなかった場合のフォールバック: 現在のファイルから3階層上（src/BitstreamGen/SAPR -> src）
    if src_path is None:
        src_path = current_file.parent.parent.parent
    
    if not src_path.exists() or not (src_path / "BitstreamGen").exists():
        raise RuntimeError(
            f"src directory not found. Current file: {current_file}, "
            f"Calculated path: {src_path}, "
            f"BitstreamGen exists: {(src_path / 'BitstreamGen').exists() if src_path.exists() else False}"
        )
    ...
    try:
        ...
        from BitstreamGen.SatPR.SATVar import BindVar as EFBindVar, ConnectVar as EFConnectVar, Var as EFVar
        import pyverilog.vparser.ast as defast
        
        # SimpleVarManagerをefpga_toolsのSATVarモジュールに追加
        from BitstreamGen.SatPR import SATVar as SATVarModule
        if not hasattr(SATVarModule, 'SimpleVarManager'):
            class SimpleVarManager:
                """SATmgrの代わりに使う簡易マネージャー（SATソルバーを実行しない）"""
                def __init__(self):
                    self.vars = {}  # count -> Var の辞書
            SATVarModule.SimpleVarManager = SimpleVarManager
        
        print("[BITSTREAM_GEN] Starting bitstream generation...", flush=True)
        
        # 1. .netファイルの生成（BitstreamGenerator用）
        ...
        # 注意: PAEArrayFromFileは既にEnumerateInterconnects()を呼び出している
        ...
        # 6-1. GEN_{番号}_パターンのPAEインスタンスをastmapに追加
        ...
        if gen_pae_instances:
            ...
            if module_def is not None:
                ...
                if high_clamp_name is None:
                    print("[WARN] High clamp signal (1'b1) not found in mapped.v. Using default.", flush=True)
                    # デフォルトのhighClamp信号名を探す（一般的な名前）
                    for item in module_def.items:
                        if isinstance(item, defast.Assign):
                            if hasattr(item.right, 'var') and hasattr(item.right.var, 'value'):
                    ...
                    if gen_pae_name in netlist.PAEInstances:
                        ...
                        for i in range(4):
                            if i in pae_instance.inputs:
                                ...
                                # 未使用の入力はhighClampに置き換える
                                if high_clamp_name:
                                    input_signals.append(high_clamp_name)
                                else:
                                    input_signals.append("1'b1")  # フォールバック
                        ...
                        print(f"[BITSTREAM_GEN] Added GEN PAE instance: {gen_pae_name} (MODE=8'h00)", flush=True)
        
        # 7. PAEInstanceのhasFFsを設定
        ...
        # 8. trueVarsの構築（SATmgrを使わずに直接構築）
        print("[BITSTREAM_GEN] Building trueVars from bind_vars_info and connect_vars_info (without SATmgr)...", flush=True)
        var_mgr = SATVarModule.SimpleVarManager()
        trueVars = []
        
        # 名前からオブジェクトを探すためのマップを作成
        pae_cell_map = {cell.name: cell for cell in pa.PAECells}
        pi_map = {pi.name: pi for pi in pa.PIs}
        po_map = {po.name: po for po in pa.POs}
        
        # PAEArrayのInterconnectから名前でオブジェクトを探すマップを作成
        interconnect_src_map = {}  # src_name -> src_obj
        interconnect_omux_map = {}  # omux_name -> omux_obj
        interconnect_dst_map = {}  # dst_name -> dst_obj
        
        for ic in pa.Interconnects:
            if ic.src is not None:
                interconnect_src_map[ic.src.name] = ic.src
            if ic.omux is not None:
                interconnect_omux_map[ic.omux.name] = ic.omux
            if ic.dst is not None:
                interconnect_dst_map[ic.dst.name] = ic.dst
        
        # bind_vars_infoからBindVarを構築
        for kind, instance_name, target_name in bind_vars_info:
            if kind == "PAE":
                # PAEInstanceとPAECellを取得
                if instance_name in netlist.PAEInstances:
                    instance = netlist.PAEInstances[instance_name]
                    # PAECellを探す
                    target = pae_cell_map.get(target_name)
                    if target is not None:
                        bind_var = EFBindVar(var_mgr, instance, target)
                        trueVars.append(bind_var)
            elif kind == "PI":
                # NetlistPIとPIを取得
                if instance_name in netlist.netlistPIs:
                    instance = netlist.netlistPIs[instance_name]
                    # PIを探す
                    target = pi_map.get(target_name)
                    if target is not None:
                        bind_var = EFBindVar(var_mgr, instance, target)
                        trueVars.append(bind_var)
            elif kind == "PO":
                ...
                if instance_name in netlist.netlistPOs:
                    instance = netlist.netlistPOs[instance_name]
                    # POを探す
                    target = po_map.get(target_name)
                    if target is not None:
                        bind_var = EFBindVar(var_mgr, instance, target)
                        trueVars.append(bind_var)
        
        # connect_vars_infoからConnectVarを構築
        for etype, src_str, omux_str, dst_str in connect_vars_info:
            # 名前からオブジェクトを取得
            src_obj = interconnect_src_map.get(src_str)
            omux_obj = interconnect_omux_map.get(omux_str) if omux_str is not None else None
            ...
            # ConnectVarを構築
            connect_var = EFConnectVar(var_mgr, src_obj, omux_obj, dst_obj)
            trueVars.append(connect_var)
        ...
        # allff.xmlの場合、pae_to_ff_mapはNoneまたは空の辞書になる
        bg = BitstreamGenerator(trueVars, astmap, l_Clamp, glueGen, pae_to_ff_map=None)
